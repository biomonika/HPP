...
workflow estimateTelomeres {
  input {
    File big_bam_file
    File big_bam_index
    String prefix = "chr"  # Prefix to filter chromosomes
    Int flank = 100000
  ...
  call SplitBamByChromosome {
        input:
            bam_file = big_bam_file,
            big_bam_index = big_bam_index,
            prefix = prefix,
            preemptible=preemptible
  } 

  scatter (bam_file in SplitBamByChromosome.selected_bams) {
...
task SplitBamByChromosome {
    ...
    command <<<
        ...
        # Extract chromosomes with the given prefix and split BAM file
        samtools idxstats ~{bam_file} | grep "^~{prefix}" | cut -f1 | grep -v "random" | grep -v "chrUn" | grep -v "EBV" > chr_list.txt
        mkdir -p split_bams
        for chr in $(cat chr_list.txt); do
            samtools view --threads ~{threadCount} -b ~{bam_file} ${chr} > split_bams/${chr}.bam
        done
    
    >>>

    output {
        Array[File] selected_bams = glob("split_bams/*.bam")
    }
