...
###PLOT TO BE USED IN THE SUPPLEMENT
p1<-ggplot(combined_data, aes(x = group, y = value/1000, fill = group)) +
  geom_boxplot(position = position_dodge(width = 0.8)) +
  labs(title = "A Combined palindromes", # of Bonobo, Chimpanzee, Gorilla, S. Orangutan, B. Orangutan, Siamang
       x = "Palindrome arm length",
       y = "Value") +
...
assign(paste0("p1"), p1, envir = .GlobalEnv)
...
generate_plots <- function(data, chromosome) {
  ...
  quarter <-
    cut(
      df$spacerlength,
      breaks = quantile(df$spacerlength, probs = c(0, 0.25, 0.5, 0.75, 1)),
  ...
  ggplot(df, aes(x = quarter, y = armlength / 1000, fill = quarter)) +
    ...
    labs(
      x = "Spacer Length [kb]",
      y = "Arm Length [kb]",
      title = paste0(chromosome, " Spacer Length vs Arm Length")
    ) +
    theme_minimal() +
    theme(
  ...
  ###PLOT TO BE USED IN THE SUPPLEMENT
  p4 <- ggplot(df, aes(x = quarter, y = gc, fill = quarter)) +
    geom_boxplot() +
    geom_point(
      position = position_jitter(width = 0.1),
      bg = "chartreuse",
    ...
    labs(
      ...
      title = paste0("C ",chromosome, " Spacer Length vs GC Content of arms")
    ) +
    theme_minimal() +
    theme(
      axis.title = element_text(size = 14),
    ...
    theme_minimal()
  ggsave(paste0("spacer_length_vs_GC_arms.", chromosome, ".png"))
  assign(paste0(chromosome, "p4"), p4, envir = .GlobalEnv)
  
  
  quarter <-
    cut(
      df$armlength,
      breaks = quantile(df$armlength, probs = c(0, 0.25, 0.5, 0.75, 1)),
      labels = c("Q1", "Q2", "Q3", "Q4"),
      include.lowest = TRUE
    )
  ggplot(df, aes(x = quarter, y = gc, fill = quarter)) +
    ...
    labs(
      x = "Arm Length [kb]",
      y = "GC Content of arms",
      title = paste0(chromosome, " Arm Length vs GC Content of arms")
    ) +
    theme_minimal() +
    theme(
      axis.title = element_text(size = 14),
  ...
  quarter <-
    cut(
      df$spacergc,
      breaks = quantile(df$spacergc, probs = c(0, 0.25, 0.5, 0.75, 1)),
      labels = c("Q1", "Q2", "Q3", "Q4"),
      include.lowest = TRUE
    )
  ggplot(df, aes(x = quarter, y = gc, fill = quarter)) +
    ...
    labs(
      x = "GC Content of spacer",
      y = "GC Content of arms",
      title = paste0(chromosome, " GC Content of spacer vs GC Content of arms")
    ) +
    theme_minimal() +
    theme(
      axis.title = element_text(size = 14),
  ...
  ###PLOT TO BE USED IN THE SUPPLEMENT
  p2 <- ggplot(df, aes(x = quarter, y = identity, fill = quarter)) +
    geom_boxplot() +
    geom_point(
      position = position_jitter(width = 0.1),
      bg = "chartreuse",
    ...
    labs(x = "Spacer Length [kb]",
         y = "Sequence identity of arms",
         title = paste0("B ",chromosome)) +
    theme_minimal() +
    theme(
      axis.title = element_text(size = 14),
      # Increase axis title size
    ...
    theme_minimal()
  ggsave(paste0("spacer_length_vs_arms_identity.", chromosome, ".png"))
  assign(paste0(chromosome, "p2"), p2, envir = .GlobalEnv)
  ...
  ###PLOT TO BE USED IN THE SUPPLEMENT
  p3 <- ggplot(df, aes(x = quarter, y = identity, fill = quarter)) +
    geom_boxplot() +
    geom_point(
      position = position_jitter(width = 0.1),
      bg = "chartreuse",
    ...
    theme(plot.background = element_rect(fill = "white"),
          plot.title = element_text(size = 13)) +
    theme_minimal()
  ggsave(paste0("arm_length_vs_arms_identity.", chromosome, ".png"))
  assign(paste0(chromosome, "p3"), p3, envir = .GlobalEnv)
  
  
  quarter <-
    cut(
      df$gc,
      breaks = quantile(df$gc, probs = c(0, 0.25, 0.5, 0.75, 1)),
      labels = c("Q1", "Q2", "Q3", "Q4"),
      include.lowest = TRUE
    )
  ggplot(df, aes(x = quarter, y = identity, fill = quarter)) +
    ...
    labs(
      x = "GC Content of arms",
      y = "Sequence identity of arms",
      title = paste0(chromosome, " GC Content of arms vs Sequence identity of arms")
    ) +
    theme_minimal() +
    theme(
      axis.title = element_text(size = 14),
  ...
  ggplot(df, aes(x = quarter, y = spacergc, fill = quarter)) +
    ...
    labs(
      ...
      title = paste0(chromosome, " Spacer Length vs GC Content of spacer")
    ) +
    theme_minimal() +
    theme(
      axis.title = element_text(size = 14),
...
generate_plots <- function(data, chromosome) {
  ...
  ggplot(df, aes(x = spacerlength/1000, y = identity)) +
    ...
    labs(x = "Spacer Length [kb]", y = "Sequence identity of arms", title = paste0("C ",chromosome)) +
    theme_minimal() +
    theme(
      axis.title = element_text(size = 14),  # Increase axis title size
      axis.text = element_text(size = 14),   # Increase axis text size
  ...
  p5<-ggplot(df, aes(x = spacerlength/1000, y = gc)) +
    geom_point(position = position_jitter(width = 0.1), pch=21, size=2) +
    labs(x = "Spacer Length [kb]", y = "GC content of arms", title = paste0("C ",chromosome)) +
    theme_minimal() +
    theme(
      axis.title = element_text(size = 14),  # Increase axis title size
      axis.text = element_text(size = 14),   # Increase axis text size
    ...
    theme(plot.background = element_rect(fill = "white"),plot.title = element_text(size = 13)) +
    facet_wrap(~species,scales="free") + theme_minimal()
  ggsave(paste0("by_species.spacer_length_vs_gc_content_of_arms.",chromosome,".png"))
  assign(paste0(chromosome, "p5"), p5, envir = .GlobalEnv)
...
grid.arrange(p1, `Chromosome Yp2`, chrXp5, `Chromosome Yp3`, ncol = 2, padding=10)
@
