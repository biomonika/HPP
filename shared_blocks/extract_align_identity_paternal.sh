...
tsv_file="shared_contigs_paternal_coordinates.tsv"
ref_pan011="assembly.v1.0.PAN011.diploid.fa"
...
output_dir="paternal_blocks"
mkdir -p "$output_dir"

# Temporary file for sequences from each haplotype
pan011_seq="$output_dir/PAN011.fa"
...
while IFS= read -r line; do
  ...
  # Parse coordinates from each haplotype (PAN011, PAN027, PAN028)
  ...
  samtools faidx "$ref_pan011" "$hap1_chr:$hap1_range" > "$pan011_seq"
  ...
  minimap2 -t ${numCPU} --cs -x asm5 -c "$pan011_seq" "$pan027_seq" > "$alignment_file" 2> /dev/null
  ...
  minimap2 -t ${numCPU} --cs -x asm5 -c "$pan028_seq" "$pan011_seq" > "$alignment_file" 2> /dev/null
  paftools.js stat "$alignment_file" >${stat_file}
  #rm $alignment_file

  #delete the unnecessary files
  rm "$pan011_seq" "$pan027_seq" "$pan028_seq"
