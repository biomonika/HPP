...
while IFS= read -r line; do
  ...
  minimap2 -t ${numCPU} --cs -x asm5 -c "$pan027_seq" "$pan011_seq" > "$alignment_file" 2> /dev/null
  ...
  minimap2 -t ${numCPU} --cs -x asm5 -c "$pan011_seq" "$pan028_seq" > "$alignment_file" 2> /dev/null
  paftools.js stat "$alignment_file" >${stat_file}
  #rm $alignment_file
