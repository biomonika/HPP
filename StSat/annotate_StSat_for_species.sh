...
awk '{print $3 "\t" $4 "\t" $5 "\t" $2 "\t" $9 "\t" $6}' ${assembly_name}.overlaps2.txt.unique.entries.txt >NCRF.${assembly_name}.bed
awk '{print $3 "\t" $4 "\t" $5 "\t" $2 "\t" $9 "\t" $6}' ${assembly_name}.overlaps2.txt.unique.entries.txt >NCRF.${assembly_name}.bed
cat NCRF.${assembly_name}.bed | awk -F'\t' 'BEGIN{SUM=0}{ SUM+=$3-$2 }END{print SUM}'
echo "Done."
